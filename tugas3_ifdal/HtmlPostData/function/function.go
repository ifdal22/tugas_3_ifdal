...
func RouteSubmitPost(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		stmt, err := db.Prepare("INSERT INTO employees (EmployeeID,LastName,FirstName,Title,TitleOfCourtesy,BirthDate,HireDate,Address,City,Region,PostalCode,HomePhone,Extension,Photo,Notes,ReportsTo,ProvinceName) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")

		_, err = stmt.Exec(EmployeeID, LastName, FirstName, Title, TitleOfCoutesy, BirthDate, HireDate, Adress, City, Region, PostalCode, Country, HomePhone, Extension, Photo, Notes, ReportsTo, ProvinceName)
		if err != nil {
